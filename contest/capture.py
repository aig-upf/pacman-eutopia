...
def runGames(layouts, agents, display, length, numGames, record, numTraining, redTeamName, blueTeamName,
             muteAgents=False, catchExceptions=False, delay_step=0, game_id=0):
  ...
  for i in range( numGames ):
    ...
    if record:
      ...
      with open(f'game_{game_id}.replay', 'wb') as f:
        f.write(g.record)
...
def save_score(game, game_id=0 ):
    with open(f'game_{game_id}.score', 'w') as f:
...
if __name__ == '__main__':
  ...
  if games:
    save_score(games[0], options['game_id'])
